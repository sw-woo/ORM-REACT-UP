...
//next.js 에서 제공해주는 SSG(static site Generation) 타입 사용하기
...
export default function Index({result}: InferGetStaticPropsType<typeof getStaticProps>) {
	...
	//client data fetching을 이용해 벡엔드 API에서 데이터를 불러와 데이터바인딩 해본다.
	...
	useEffect(() => {
		...
		const getSingleBlog = async () => {
			const res = await fetch("http://localhost:3005/api/articles/15", {
				method: "GET",
				headers: {
