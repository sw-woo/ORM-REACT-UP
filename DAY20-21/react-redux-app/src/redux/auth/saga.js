...
//벡엔드 RESTful통신을 위한 APIClient post메소드(create()) 함수 생성하기
const create = new APIClient().create;

//로그인 벡엔드 통신 처리를 위한 제너레이터 함수 정의
//로그인처리 SAGA 제너레이터 함수
...
//제너레이터 함수를 노출합니다.
//watchLoginUser함수를 정의하고 기능을 노출하면 추후 관련 액션함수가 실행되면 자동으로 saga제너레이터 함수가 실행됩니다.
export function* watchLoginUser() {
	//takeEvery함수의 목적은 전달되는 액션타입별, 액션함수를 실행시켜주는 기능제공
...
function* authSaga() {
	...
	//all함수는 실행해야할 사가함수들을 배열에 넣고 여러개를 동시에 실행시킬수 있는 기능 제공
